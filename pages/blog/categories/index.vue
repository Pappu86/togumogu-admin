    ...
    getCategoriesByPagination: debounce(async function () {
      ...
      try {
        const { data, meta } = await this.$axios.$get(
          `${this.language}/blog/category?${url}`
        );
        this.loading = false;
    ...
    async updateParentChild() {
      try {
        const { message } = await this.$axios.$post(
          `${this.language}/blog/category-rebuild`,
          this.newCategories
        );
        this.getCategoriesByPagination();
    ...
    async create() {
      ...
      try {
        const { categoryId } = await this.$axios.$post(
          `${process.env.API_URL}/${this.language}/blog/category`
        );
        this.creating = false;
    ...
    async deleteCategory(id) {
      try {
        const { message } = await this.$axios.$delete(
          `${this.language}/blog/category/${id}`
        );
        this.getCategoriesByPagination();
        this.$store.dispatch('crud_success_dialog', message);
      } catch (e) {
