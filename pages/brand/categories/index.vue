                ...
                <template slot-scope="{ item }">
                  ...
                  <div class="nestable-action">
                    <v-btn
                      ...
                      small
                      title="Edit"
                      :to="`/brand/categories/${item.id}`"
                      nuxt
                    >
                      <v-icon>mdi-pencil</v-icon>
                    </v-btn>
                    <v-btn
                      :v-can="`delete brand_category`"
                      depressed
                      outlined
                      icon
                      fab
    ...
    getCategoriesByPagination: debounce(async function () {
      ...
      try {
        const { data, meta } = await this.$axios.$get(
          `${this.language}/brand/category?${url}`
        );
        this.loading = false;
    ...
    async updateParentChild() {
      try {
        const { message } = await this.$axios.$post(
          `${this.language}/brand/category-rebuild`,
          this.newCategories
        );
        this.getCategoriesByPagination();
    ...
    async create() {
      ...
      try {
        const { categoryId } = await this.$axios.$post(
          `${this.language}/brand/category`
        );
        this.creating = false;
        this.$router.push(`/brand/categories/${categoryId}`);
      } catch (e) {
        this.creating = false;
        this.$store.commit(
          'APP_NETWORK_ERROR',
    ...
    async deleteCategory(id) {
      try {
        const { message } = await this.$axios.$delete(
          `${this.language}/brand/category/${id}`
        );
        this.getCategoriesByPagination();
        this.$store.dispatch('crud_success_dialog', message);
      } catch (e) {
