    ...
    async updateCategory() {
      ...
      try {
        const { message } = await this.$axios.$patch(
          `${this.language}/daycare/category/${this.$route.params.id}`,
          this.category
        );
        this.loading = false;
        this.$store.dispatch('crud_success', {
          message,
          path: '/daycare/categories',
        });
      } catch (err) {
        this.loading = false;
    ...
    async checkSlug() {
      try {
        const { slug } = await this.$axios.$get(
          `${this.language}/daycare/category/slug/${encodeURIComponent(
            this.category.name
          )}`
        );
