              ...
              >
                ...
                <template v-slot:item.action="{ item }">
                  <edit-in-same-page-actions
                    update="update daycare_category"
                    delete-action="delete daycare_category"
                    :to="`/daycare/categories/${item.id}`"
...
export default {
  ...
  methods: {
    ...
    getCategoriesByPagination: debounce(async function () {
      ...
      try {
        const { data } = await this.$axios.$get(
          `${this.language}/daycare/category?${url}`
        );
        this.loading = false;
    ...
    async create() {
      this.creating = true;
      try {
        const { daycareCategoryId } = await this.$axios.$post(
          `${this.language}/daycare/category`
        );
        this.creating = false;
        this.$router.push(`/daycare/categories/${daycareCategoryId}`);
      } catch (e) {
        this.creating = false;
        this.$store.commit(
          'APP_NETWORK_ERROR',
    ...
    async deleteCategory(id) {
      try {
        const { message } = await this.$axios.$delete(
          `${this.language}/daycare/category/${id}`
        );
        this.getCategoriesByPagination();
