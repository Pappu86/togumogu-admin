                ...
                <template slot-scope="{ item }">
                  ...
                  <div class="nestable-action">
                    <v-btn
                      ...
                      small
                      title="Edit"
                      :to="`/media/categories/${item.id}`"
                      nuxt
                    >
                      <v-icon>mdi-pencil</v-icon>
                    </v-btn>
                    <v-btn
                      :v-can="`delete asset_category`"
                      depressed
                      outlined
                      icon
                      fab
...
export default {
  ...
  methods: {
    ...
    getCategoriesByPagination: debounce(async function () {
      ...
      try {
        const { data, meta } = await this.$axios.$get(`asset/category?${url}`);
        this.loading = false;
    ...
    async create() {
      this.creating = true;
      try {
        const { categoryId } = await this.$axios.$post(`asset/category`);
        this.creating = false;
        this.$router.push(`/media/categories/${categoryId}`);
      } catch (e) {
        this.creating = false;
        this.$store.commit(
          'APP_NETWORK_ERROR',
    ...
    async deleteCategory(id) {
      try {
        const { message } = await this.$axios.$delete(`asset/category/${id}`);
        this.getCategoriesByPagination();
        this.$store.dispatch('crud_success_dialog', message);
      } catch (e) {
        this.$store.commit(
