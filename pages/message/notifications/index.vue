              ...
              >
                ...
                <template v-slot:item.action="{ item }">
                  <v-btn
                    ...
                    color="primary"
                    title="View"
                    :to="`/message/notifications/${item.id}`"
                  >
                  ...
                  <v-btn
                    ...
                    color="teal"
                    title="Edit"
                    :to="`/message/notifications/${item.id}/edit?edit=true&lang=${language}`"
                  >
    ...
    getNotificationsByPagination: debounce(async function () {
      ...
      try {
        const { data, meta } = await this.$axios.$get(
          `${this.language}/message/notification?${url}`
        );
        this.loading = false;
    ...
    async createNotification() {
      ...
      try {
        const { notificationId } = await this.$axios.$post(
          `${this.language}/message/notification`
        );
        this.creating = false;
        if (notificationId) {
          this.$router.push(`/message/notifications/${notificationId}/edit`);
        }
      } catch (e) {
        this.creating = false;
        this.$store.commit(
