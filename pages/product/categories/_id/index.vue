              ...
              <v-card>
                <v-card-text>
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" sm="6">
                      <b-text
                        v-model="category.google_category_id"
                        rules="numeric"
                        outlined
                        dense
                    ...
                    <v-col cols="12" sm="6">
                      <b-text
                        v-model="category.google_category_name"
                        outlined
                        dense
...
export default {
  ...
  methods: {
    ...
    async checkSlug() {
      try {
        const { slug } = await this.$axios.$get(
          `${this.language}/product/category/slug/${encodeURIComponent(
            this.category.name
          )}`
        );
    ...
    async updateCategory() {
      ...
      try {
        const { message } = await this.$axios.$patch(
          `${process.env.API_URL}/${this.language}/product/category/${this.$route.params.id}`,
          this.category
        );
        this.loading = false;
    ...
    async getCategories() {
      try {
        const { data } = await this.$axios.$get(
          `${process.env.API_URL}/${this.language}/product/category-all`
        );
