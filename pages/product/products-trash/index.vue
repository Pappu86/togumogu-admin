    ...
    getProductsByPagination: debounce(async function () {
      ...
      try {
        const { data, meta } = await this.$axios.$get(
          `${process.env.API_URL}/${this.language}/product/product-trashed?${url}`
        );
        this.loading = false;
        this.products = data;
    ...
    async deleteProduct(id) {
      try {
        const { message } = await this.$axios.$delete(
          `${process.env.API_URL}/${this.language}/product/product-force/${id}`
        );
        this.getProductsByPagination();
    ...
    async restoreItem(id) {
      try {
        const { message } = await this.$axios.$get(
          `${process.env.API_URL}/${this.language}/product/product-trashed/${id}`
        );
        this.getProductsByPagination();
        this.$store.commit('CRUD_SUCCESS_NO_ACTION', message);
      } catch (e) {
