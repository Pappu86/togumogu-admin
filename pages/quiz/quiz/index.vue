<template>
  <div id="quizList">
    <v-container grid-list-xl fluid>
      <v-layout row wrap>
        <v-flex xs12>
          <div style="display: flex">
            <h3>Quiz</h3>
            <v-spacer />
            <v-btn
              v-can="`create quiz`"
              :loading="creating"
              color="primary"
              @click="createQuiz"
              >Create Quiz</v-btn
            >
          </div>
        </v-flex>
        <v-flex xs12>
          <v-card>
            <v-card-title class="pa-1">
              <v-text-field
                v-model="search"
                solo
                flat
                prepend-inner-icon="mdi-magnify"
                placeholder="Type something"
                hide-details
              ></v-text-field>
              <div class="d-flex">
                <b-language v-model="language" class="mt-1" />
              </div>
            </v-card-title>
            <v-divider />
            <v-card-text class="pa-0">
              <v-data-table
                v-model="selected"
                :headers="headers"
                :items="quizzes"
                :options.sync="pagination"
                :server-items-length="total"
                :loading="loading"
                show-select
                class="elevation-1"
              >
                <template v-slot:item.area="{ item }">{{
                  getAreaNames(item.area)
                }}</template>
                <template v-slot:item.platforms="{ item }">{{
                  getPlatforms(item.platforms)
                }}</template>
                <template v-slot:item.status="{ item }">
                  <v-chip
                    class="white--text"
                    :color="item.status === 'active' ? 'success' : 'warning'"
                  >
                    {{ item.status }}
                  </v-chip>
                </template>
                <template v-slot:item.action="{ item }">
                  <v-btn
                    v-can="`view quiz`"
                    icon
                    small
                    fab
                    color="primary"
                    title="View"
                    :to="`/quiz/quiz/${item.id}`"
                  >
                    <v-icon>mdi-eye</v-icon>
                  </v-btn>
                  <v-btn
                    v-can="`update quiz`"
                    icon
                    small
                    fab
                    color="teal"
                    title="Edit"
                    :to="`/quiz/quiz/${item.id}/edit?edit=true&lang=${language}`"
                  >
                    <v-icon>mdi-pencil</v-icon>
                  </v-btn>
                  <v-btn
                    v-can="`delete quiz`"
                    icon
                    small
                    fab
                    color="pink"
                    title="Delete"
                    @click="deleteItem(item)"
                  >
                    <v-icon>mdi-delete</v-icon>
                  </v-btn>
                </template>
              </v-data-table>
            </v-card-text>
          </v-card>
        </v-flex>
      </v-layout>
    </v-container>
  </div>
</template>

<script>
import { debounce } from 'lodash-es';
import { confirmationAlert } from '@/plugins/sweetalert';

export default {
  name: 'Quiz',
  data() {
    return {
      language: 'en',
      quizzes: [],
      selected: [],
      search: '',
      total: 0,
      loading: false,
      creating: false,
      pagination: {},
      headers: [
        { text: 'Date', value: 'created_at', width: 130 },
        { text: 'Title', value: 'title', sortable: false },
        { text: 'Area', value: 'area', width: 100 },
        { text: 'Platform', value: 'platforms', width: 100 },
        { text: 'Max Time (Second)', value: 'max_time' },
        { text: 'Total Point', value: 'total_points' },
        { text: 'Reward Points', value: 'reward_points' },
        { text: 'Status', value: 'status' },
        { text: 'Actions', value: 'action', sortable: false, align: 'center' },
      ],
      itemsPerPageItems: [10, 20, 50, 100],
      getAreaNames(areaList) {
        const areas = JSON.parse(areaList);
        const total = (areas && areas.length) || 0;
        let names = '';
        if (total > 0) {
          areas.forEach((areaInfo, index) => {
            if (areaInfo) {
              names += `${areaInfo.name}${total - 1 !== index ? ', ' : ''}`;
            }
          });
          return names;
        } else return '-';
      },
      getPlatforms(platforms) {
        const platformArray = JSON.parse(platforms);
        const total = (platforms && platforms.length) || 0;
        if (total > 0) {
          let names = '';
          platformArray.forEach((platform, index) => {
            if (platform) {
              names += `${platform}${total - 1 !== index ? ', ' : ''}`;
            }
          });
          return names;
        } else return '-';
      },
    };
  },
  watch: {
    // for server side pagination
    pagination() {
      this.getQuizzesByPagination();
    },
    // for server side search
    search() {
      this.getQuizzesByPagination();
    },
    language() {
      this.getQuizzesByPagination();
    },
  },
  mounted() {
    window.getApp.$on('REFRESH_PAGE_CONTENT', () => {
      this.getQuizzesByPagination();
    });
  },
  beforeDestroy() {
    window.getApp.$off('REFRESH_PAGE_CONTENT');
  },
  methods: {
    // get quiz by pagination
    getQuizzesByPagination: debounce(async function () {
      this.loading = true;
      let url = `page=${this.pagination.page}&per_page=${this.pagination.itemsPerPage}`;
      // get by search keyword
      if (this.search) {
        url += `&query=${this.search}`;
      }

      // get by sort option
      if (this.pagination.sortBy.length && !this.search) {
        const direction = this.pagination.sortDesc[0] ? 'desc' : 'asc';

        url += `&direction=${direction}&sortBy=${this.pagination.sortBy[0]}`;
      }

      try {
        const { data, meta } = await this.$axios.$get(
          `${process.env.API_URL}/${this.language}/quiz/quiz?${url}`
        );
        this.loading = false;
        this.quizzes = data;
        this.total = meta.total;
      } catch (e) {
        this.loading = false;

        this.$store.commit(
          'APP_NETWORK_ERROR',
          e.response && e.response.data.message
        );
      }
    }, 500),
    // create quiz
    async createQuiz() {
      this.creating = true;
      try {
        const { quizId } = await this.$axios.$post(
          `${process.env.API_URL}/${this.language}/quiz/quiz`
        );
        this.creating = false;
        this.$router.push(`/quiz/quiz/${quizId}/edit`);
      } catch (e) {
        this.creating = false;
        this.$store.commit(
          'APP_NETWORK_ERROR',
          e.response && e.response.data.message
        );
      }
    },
    // delete quiz
    deleteItem(item) {
      confirmationAlert().then((res) => {
        if (res.value) {
          this.deleteQuiz(item.id);
        }
        return false;
      });
    },
    async deleteQuiz(id) {
      try {
        const { message } = await this.$axios.$delete(
          `${process.env.API_URL}/${this.language}/quiz/quiz/${id}`
        );
        this.getQuizzesByPagination();
        this.$store.commit('CRUD_SUCCESS_NO_ACTION', message);
      } catch (e) {
        this.$store.commit(
          'APP_NETWORK_ERROR',
          e.response && e.response.data.message
        );
      }
    },
  },
};
</script>
