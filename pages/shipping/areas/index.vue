          ...
          <div style="display: flex">
            ...
            <v-dialog v-model="dialog" max-width="500px" persistent>
              <template v-slot:activator="{ on }">
                <v-btn :v-can="`create area`" color="primary" v-on="on">
                  New Area
                </v-btn>
              </template>
              <v-card>
...
export default {
  ...
  methods: {
    // get area by pagination
    getAreasByPagination: debounce(async function () {
      this.loading = true;
      ...
      try {
        const { data, meta } = await this.$axios.$get(`shipping/area?${url}`);
        this.loading = false;
        this.areas = data;
    ...
    onSubmit() {
      ...
      if (this.editedIndex > -1) {
        url = `shipping/area/${this.editedItem.id}`;
        method = 'PATCH';
    ...
    async deleteArea(id) {
      try {
        const { message } = await this.$axios.$delete(
          `${process.env.API_URL}/shipping/area/${id}`
        );
        this.getAreasByPagination();
        this.$store.dispatch('crud_success_dialog', message);
      } catch (e) {
    ...
    async getDistricts() {
      try {
        const { data } = await this.$axios.$get(`shipping/district-all`);
        this.districts = data;
      } catch (e) {}
    },
  },
