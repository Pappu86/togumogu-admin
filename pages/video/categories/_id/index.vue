    ...
    async updateCategory() {
      ...
      try {
        const { message } = await this.$axios.$patch(
          `${process.env.API_URL}/${this.language}/video/category/${this.$route.params.id}`,
          this.category
        );
        this.loading = false;
    ...
    async checkSlug() {
      try {
        const { slug } = await this.$axios.$get(
          `${this.language}/video/category/slug/${encodeURIComponent(
            this.category.name
          )}`
        );
