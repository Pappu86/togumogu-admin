...
const state = () => ({
  ...
  settings: {
    site_name: 'ToguMogu Admin',
    footer_text: 'ToguMogu Admin.',
...
const mutations = {
  ...
  [PRINTABLE_DATA_UPDATED](state, payload) {
    state.collections = payload;
  },
  [APP_GENERAL_ERROR](state, payload) {
    state.snackbar = {
      show: true,
      message: payload,
      color: 'error',
...
const actions = {
  ...
  async get_common_settings({ commit }) {
    try {
      const { data } = await this.$axios.$get(`common-settings`);
